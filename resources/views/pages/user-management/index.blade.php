        ...
        <div class="card-body pt-6">
            ...
            <tbody>
                @foreach ($users as $user)
                    ...
                    <td class="flex">
                        <form action="/user-management/edit" method="GET">
                            @csrf
                            @method('GET')
                        <input type="hidden" value="{{ $user->id }}" name="id" />
                        <button type="submit" value="{{ $user->id }}" class="btn btn-alert" >Edit</button>

                        </form>&nbsp;&nbsp;&nbsp;<form action="/user-management/delete" method="POST">
                            @csrf
                            @method('DELETE')
                            <input type="hidden" name="id" value="{{ $user->id }}" />
                            <input type="hidden" name="email" value="{{ $user->email }}" />
                            <button class=" btn btn-danger" value="{{ $user->id }}">Delete</button></form>
